...
import { GenerosRepository } from './repositories/generos.repository.js';
...
import { GenerosService } from './services/generos.service.js';
...
export function createContainer(database, config) {
  ...
  const videojuegos = new VideojuegosRepository(database);
  const generos = new GenerosRepository(database);
  ...
  return {
    ...
    jugadores: new JugadoresService(jugadores), videojuegos: new VideojuegosService(videojuegos, generos),
    generos: new GenerosService(generos),
