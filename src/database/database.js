...
export class Database {
  ...
  async verify() {
    ...
    const [tables] = await this.execute(
      "SELECT TABLE_NAME, ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME IN ('jugadores','videojuegos','puntuaciones','api_solicitudes','usuarios','roles', 'generos')",
    );
    if (tables.length !== 7 || tables.some(t => t.ENGINE !== 'InnoDB')) throw new Error('Faltan tablas o no usan InnoDB; revisa DB_NAME y las tablas.');
    await this.execute('SELECT ID, Nombre, Correo, Contrasena, ID_Rol FROM usuarios LIMIT 0');
    const [roles] = await this.execute("SELECT Nombre FROM roles WHERE Nombre IN ('SUPER ADMINISTRADOR', 'ADMINISTRADOR')");
    ...
    await this.execute('SELECT ID, nombre, ID_genero FROM videojuegos LIMIT 0');
    ...
    await this.execute('SELECT ID, nombre FROM generos LIMIT 0');
  }
